...
export default function SeasonLeaders({ league = "nba" }: { league?: LeagueId }) {
  const [data, setData] = useState<SeasonLeaders | null>(null);
  const [season, setSeason] = useState("");
  useEffect(() => { loadSeasonLeaders(league).then((d) => { setData(d); setSeason(Object.keys(d.bySeason).sort().reverse()[0] || ""); }).catch(() => {}); }, [league]);
                ...
                {list.slice(0, 10).map((p, i) => {
                  ...
                  return (
                    <li key={p.pid} style={{ display: "flex", gap: 8, alignItems: "center", fontSize: ".86rem" }}>
                      ...
                      <Link href={leagueHref(league, `/players/${p.pid}/${slugify(p.name)}`)} style={{ flex: 1, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.name}</Link>
