...
export const metadata = pageMeta({
  title: "WNBA Stat Leaders — points, rebounds, assists & more",
  description: "Per-season and career per-game leaders across the dataset: points, rebounds, assists, steals, blocks, threes, true shooting and more. Built from real WNBA box-score data.",
  path: "/wnba/stats",
  keywords: ["WNBA stats", "WNBA scoring leaders", "WNBA stat leaders", "most WNBA points", "WNBA season leaders"],
});
...
function safePlayers() {
  try { return allPlayers("wnba"); } catch { return []; }
}

export default function WnbaStatsPage() {
  ...
  return (
    <div style={{ display: "grid", gap: "1.5rem" }}>
      <header>
        ...
        <p style={{ color: "var(--muted)", marginTop: 6 }}>Per-season and career leaders, from real WNBA match stats.</p>
      ...
      {!hasData ? (
        <div className="card" style={{ padding: "1.5rem", color: "var(--muted)" }}>
          WNBA stat leaders arrive once the season pipeline has run.
        </div>
      ) : (
        <>
          <section style={{ display: "grid", gap: 12 }}>
            <h2 style={{ margin: 0, fontSize: "1.2rem" }}>Single-season leaders</h2>
            <SeasonLeaders league="wnba" />
          </section>
                    ...
                    {top.map((p, i) => {
                      ...
                      return (
                        <li key={p.id} style={{ display: "flex", gap: 8, alignItems: "center", fontSize: ".86rem" }}>
                          ...
                          <Link href={leagueHref("wnba", `/players/${p.id}/${p.slug}`)} style={{ flex: 1, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.name}</Link>
