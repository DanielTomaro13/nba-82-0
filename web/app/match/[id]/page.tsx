...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }) {
  ...
  return pageMeta({
    ...
    description: `Full box score and player lineups: ${g.away.name} at ${g.home.name}, ${g.date}. ${winner.name} won ${Math.max(g.home.pts, g.away.pts)}–${Math.min(g.home.pts, g.away.pts)}. Real NBA game stats.`,
    path: `/match/${id}`,
    keywords: [g.home.name, g.away.name, "NBA box score", "lineups", "game stats", g.date],
  });
}
